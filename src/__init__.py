...
def lost_time(game_CreatedAt):
    dt = datetime.fromisoformat(game_CreatedAt.replace('Z', '+00:00'))

    return int(time.time()-dt.timestamp())

def choose_based_on_percentage(percentage_true):
    random_value = random.uniform(0, 100)
    return random_value <= percentage_true